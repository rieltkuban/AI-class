...
## 6-альт. Если на входе сервера уже стоит Caddy

Проверьте, чем заняты порты:

```
ss -lptn | grep -E ':(80|443)'
```

Если там `docker-proxy` или `caddy` — **шаг 6 целиком не подходит**. nginx
не сможет занять эти порты, а certbot не нужен: Caddy выпускает и продлевает
сертификаты сам.

### 6а.1. Адрес шлюза Docker-сети

Приложение живёт на хосте, а Caddy — в контейнере. Чтобы он дотянулся
до хоста, нужен адрес шлюза его сети:
...
```
docker network inspect ИМЯ_СЕТИ | grep -i gateway
```

Обычно `172.17.0.1` или `172.18.0.1`. Прописываем его приложению:

```
sed -i 's/^BIND_HOST=.*/BIND_HOST=172.18.0.1/' /var/www/aiclass/.env.production && systemctl restart aiclass
```

Проверяем, что приложение слушает нужный адрес:

```
ss -lptn | grep 3000
```

Порт наружу при этом закрыт: `ufw` его не пропускает, а адрес шлюза виден
только из Docker-сети.

### 6а.1а. Файрвол: разрешить контейнеру достучаться до хоста

Если на сервере включён `ufw`, он пропускает только то, что разрешено явно.
Запрос из контейнера Caddy в приложение на хосте приходит по мостовому
интерфейсу, попадает в цепочку INPUT и молча отбрасывается. Снаружи это
выглядит как **502 Bad Gateway**, хотя приложение работает.

Проверка с хоста этого НЕ ловит: там запрос идёт через loopback и проходит.
Проверять надо из контейнера:

```
docker exec ИМЯ_КОНТЕЙНЕРА_CADDY wget -qO- -T 3 http://172.18.0.1:3000/api/stats
```

Пусто или ошибка — разрешаем только этот путь:

```
ufw allow from 172.18.0.0/16 to any port 3000 proto tcp
```

Правило узкое: только из Docker-сети, только на порт приложения. Из интернета
порт остаётся закрытым.

### 6а.2. Резервная копия Caddyfile — до любых правок

```
cp /opt/site/ВАШ_ПРОЕКТ/Caddyfile /opt/site/ВАШ_ПРОЕКТ/Caddyfile.backup-$(date +%F-%H%M) && ls -la /opt/site/ВАШ_ПРОЕКТ/Caddyfile*
```

### 6а.3. Блок для нашего сайта

Готовый блок лежит в `deploy/Caddyfile.snippet`, там два варианта: временный
по IP без сертификата и боевой с доменом. Возьмите нужный, замените адрес
шлюза и домен, допишите в конец существующего Caddyfile:

```
nano /opt/site/ВАШ_ПРОЕКТ/Caddyfile
```

### 6а.4. Проверка и перезагрузка на живую

Сначала валидатор — он покажет ошибку до того, как её увидят посетители:

```
docker exec ИМЯ_КОНТЕЙНЕРА_CADDY caddy validate --config /etc/caddy/Caddyfile
```

Если «Valid configuration» — перезагружаем конфиг **без перезапуска контейнера**:

```
docker exec ИМЯ_КОНТЕЙНЕРА_CADDY caddy reload --config /etc/caddy/Caddyfile
```

`caddy reload` подменяет конфиг на ходу. Существующий сайт не перезапускается
и не простаивает ни секунды.

Если валидатор ругается — верните копию из 6а.2 и ничего не перезагружайте:
работающий конфиг останется прежним.

### 6а.5. Проверка потока через Caddy

```
curl -N -X POST http://IP_ИЛИ_ДОМЕН/api/run -H 'Content-Type: application/json' -d '{"level":"none","contour":"price","cycleDays":7,"revenue":1000000000}'
```

Строки `event: delta` должны идти порциями. Если весь текст вывалился разом —
в блоке потерялся `flush_interval -1`.

**Чего в Caddy нет:** ограничения частоты на уровне прокси. Второго рубежа
не будет, лимит держит только приложение — три прогона в час с одного адреса.
Взамен Caddy сам подставляет `X-Forwarded-For` и игнорирует присланный
клиентом, так что подделать свой адрес и обойти лимит нельзя.
